...
UserController.create = async (req, res) => {
  ...
  try {
    ...
    req.flash(
      'success',
      `New user (${savedUser.name}) has been created successfully!`
    );
    return res.redirect('/users');
  } catch (error) {
    req.flash('error', 'User already exist with this email account.');
...
UserController.update = async (req, res) => {
  ...
  req.flash('success', `User (${user.name}) has been updated successfully!`);
  res.redirect('/users');
};

UserController.delete = async (req, res) => {
  const user = await User.findByIdAndDelete(req.params.id);
  req.flash('error', `User (${user.name}) has been deleted successfully!`);
  res.redirect('/users');
};

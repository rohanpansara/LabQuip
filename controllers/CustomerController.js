...
CustomerController.create = async (req, res) => {
  const { name, phone, address, amount, paid, product } = req.body;
  const validator = CustomerValidator({ name, phone, address, product });
  if (validator.error) {
  ...
  try {
    ...
    customer = new Customer({ name, phone, address, product: getProduct._id, });
    await customer.save();
    req.flash('success', 'Supplier Details has been successfully added!');
    return res.redirect('/customers');
  } catch (e) {
...
CustomerController.delete = async (req, res) => {
  ...
  req.flash('success', `Supplier has been deleted successfully!`);
...
CustomerController.update = async (req, res) => {
  ...
  if (validator.error) {
    ...
    req.flash(
      'success',
      `Supplier info for "${newCustomer.name}" has been updated successfully!`
    );
    res.redirect('/customers');
  }
};
...
CustomerController.getCustomer = async (req, res) => {
  try {
    ...
    return res.send("Supplier Doesn't Exist");
  } catch (e) {
    return '';
  }
};
